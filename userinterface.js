...
router.post('/submit_orders', function (req, res, next) {
  pool.query("insert into  orders  (orderdate,userid,mobilenumber,emailid)values(?,?,?,?)",[req.body.orderdate,req.body.userid,req.body.mobileno,req.body.emailid],function (error, result) {
      if (error) {
          console.log(error)
          return res.status(500).json({ message:'Server Error',status:false})
      }
