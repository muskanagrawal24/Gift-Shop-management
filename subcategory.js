...
router.post('/add_new_subcategory',upload.single('icon'),function(req,res){
    pool.query('insert into subcategory(categoryid,subcategoryname,icon,bannerpriority) values(?,?,?,?)',[req.body.categoryid,req.body.subcategoryname,req.file.filename,req.body.priority],function(error,result){
        if(error){
...
router.post('/edit_subcategory_data',function(req,res){
 console.log('bodyy',req.body)
    ...
    pool.query('update subcategory set categoryid=?,subcategoryname=?,bannerpriority=? where subcategoryid=?',
